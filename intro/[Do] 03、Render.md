...
## 🟢 **在 Render 上部署：**
    *   註冊 Render 帳號，並授權它訪問你的 GitHub 帳號。
    *   在 Render 儀表板上，點擊 `New +` -> `Web Service`。
    *   選擇你剛剛創建的 GitHub 倉庫。
    *   Render 會自動檢測到是 Python 專案。你需要配置以下幾項：
        *   **Root Directory:** 保持預設即可。如果剛剛的檔案放到了資料夾中，就填寫那個資料夾的名稱
        *   **Environment:** 選擇 `Python`。
        *   **Build Command:** `pip install -r requirements.txt` (通常會自動填寫)。
        *   **Start Command:** `uvicorn main:app --host 0.0.0.0 --port $PORT`
            *   **關鍵：** 必須用 `--host 0.0.0.0` 讓服務監聽所有網路接口，而不是只監聽 `127.0.0.1`。`$PORT` 是 Render 會自動注入的環境變數，你不需要手動指定端口號。

<div align="center">
  <img src="/image/image01.png" width="70%">
</div>

## 🟢 **獲取公開 URL：**
    *   點擊 `Create Web Service`，Render 就會開始構建和部署你的應用。
    *   幾分鐘後，部署完成，Render 會給你一個公開的 URL，例如 `https://my-grok-proxy.onrender.com`。

## 🟢 最後一步：在雲端 Dify 中配置

現在你有了這個公開的 URL，回到你的**雲端 Dify** 平台：

1.  進入 `設置` -> `模型供應商` -> `添加模型` (選擇 OpenAI)。
2.  **API 基礎 URL (API Base URL):** 填入你從 Render 獲取的那個公開 URL，後面記得加上 `/v1`。例如：
    *   `https://my-grok-proxy.onrender.com/v1`
3.  其他設置（API Key, 模型名稱等）和之前一樣。
